#!/bin/sh
#Version 0.1.0
#Info: Installs Desire daemon, Masternode based on privkey.
#Desire Version 0.12.2.2 or above
#Tested OS: Ubuntu 17.04, 16.04, and 14.04
#TODO: make script less "ubuntu" or add other linux flavors
#TODO: remove dependency on sudo user account to run script (i.e. run as root and specifiy desire user so desire user does not require sudo privileges)
#TODO: add specific dependencies depending on build option (i.e. gui requires QT4)

noflags() {
    echo "=-=-=-=-=-=-=-=-=-=-="
    echo "Usage: install-desire"
    echo "Example: install-desire"
    echo "=-=-=-=-=-=-=-=-=-=-="
    exit 1
}
...
compile() {
	cd desire #TODO: squash relative path
	...
	if [ $? -ne 0 ]; then error; fi
	message "Building Desire...this may take a few minutes..."
	make
	if [ $? -ne 0 ]; then error; fi
	message "Installing Desire..."
	sudo make install
	if [ $? -ne 0 ]; then error; fi
}
...
createconf() {
	...
	printf "%s\n" "rpcuser=$rpcuser" "rpcpassword=$rpcpass" "rpcallowip=127.0.0.1" "listen=1" "server=1" "daemon=1" "maxconnections=256" "rpcport=12454" "externalip=$mnip" "masternode=1" "masternodeprivkey=$MNPRIVKEY" > $CONFILE
        ...
        sleep 20s
        MNPRIVKEY=$(desire-cli masternode genkey)
	desire-cli stop
	message "wait 10 seconds for deamon to stop..."
        sleep 10s
	sudo rm $CONFILE
	message "Updating desire.conf..."
        printf "%s\n" "rpcuser=$rpcuser" "rpcpassword=$rpcpass" "rpcallowip=127.0.0.1" "listen=1" "server=1" "daemon=1" "maxconnections=256" "rpcport=12454" "externalip=$mnip" "masternode=1" "masternodeprivkey=$MNPRIVKEY" > $CONFILE
...
success() {
	desired
	message "SUCCESS! Your desired has started. Masternode.conf setting below..."
